...
func (iq *ImageQuery) sqlAll(ctx context.Context) ([]*Image, error) {
	...
	if iq.withPosts != nil || iq.withUploadedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := iq.withUploadedBy; query != nil {
		...
		nodeids := make(map[int][]*Image)
		for i := range nodes {
			if nodes[i].user_upload == nil {
				continue
			}
			fk := *nodes[i].user_upload
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_upload" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.UploadedBy = n
			}
		}
	}
