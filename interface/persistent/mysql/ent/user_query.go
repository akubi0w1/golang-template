...
func (uq *UserQuery) QueryUpload() *ImageQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUpload:  uq.withUpload.Clone(),
...
func (uq *UserQuery) WithUpload(opts ...func(*ImageQuery)) *UserQuery {
	query := &ImageQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUpload; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Upload = []*Image{}
		}
		query.withFKs = true
		query.Where(predicate.Image(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UploadColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_upload
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_upload" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_upload" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Upload = append(node.Edges.Upload, n)
		}
	}
