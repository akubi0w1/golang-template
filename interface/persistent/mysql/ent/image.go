...
func (e ImageEdges) UploadedByOrErr() (*User, error) {
	if e.loadedTypes[1] {
		if e.UploadedBy == nil {
			// The edge uploadedBy was loaded in eager-loading,
			// but was not found.
...
func (*Image) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case image.ForeignKeys[0]: // post_images
			values[i] = new(sql.NullInt64)
		case image.ForeignKeys[1]: // user_upload
			values[i] = new(sql.NullInt64)
		default:
	...
	for j := range columns {
		...
		case image.ForeignKeys[0]:
			if value, ok := values[j].(*sql.NullInt64); !ok {
				...
				i.post_images = new(int)
				*i.post_images = int(value.Int64)
			}
		case image.ForeignKeys[1]:
			if value, ok := values[j].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_upload", value)
			} else if value.Valid {
				i.user_upload = new(int)
				*i.user_upload = int(value.Int64)
			}
		}
	}
	return nil
...
// QueryUploadedBy queries the "uploadedBy" edge of the Image entity.
func (i *Image) QueryUploadedBy() *UserQuery {
	return (&ImageClient{config: i.config}).QueryUploadedBy(i)
}
